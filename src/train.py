...
def load_cifar_100_data(data_dir, negatives=False):
    ...
    # training data
    cifar_train_data = unpickle(data_dir + "/train")
    cifar_train_filenames = np.array(cifar_train_data[b'filenames'])
    cifar_train_labels = np.array(cifar_train_data[b'fine_labels']) 

    cifar_train_data = cifar_train_data[b'data']
    cifar_train_data = cifar_train_data.reshape(len(cifar_train_data),3,32,32).transpose(0,2,3,1).astype(np.float32)

    # test data
    cifar_test_data = unpickle(data_dir + "/test")
    cifar_test_filenames = np.array(cifar_test_data[b'filenames'])
    cifar_test_labels = np.array(cifar_test_data[b'fine_labels'])

    cifar_test_data = cifar_test_data[b'data']
